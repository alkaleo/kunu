...
export function Timeline() {
  ...
  return <main className="content-page"><header className="content-heading"><p className="eyebrow">Growing through journeys</p><h1>A story measured in places.</h1><p>From six to seven, every journey becomes part of Clara’s timeline.</p></header><div className="timeline-list">{journeys.map((journey, index) => <article className="timeline-entry" key={journey.id}><div className="timeline-age"><span>{journeyAge(child, journey)}</span><small>years old</small></div><button className="timeline-cover" onClick={() => setPortal(journey)}><img src={journey.coverAsset} alt={`Open ${journey.location} memory portal`}/><span>Open memory</span></button><div className="timeline-copy"><p>{formatJourneyDate(journey.date)} · {journey.theme}</p><h2>{journey.location}, {journey.country}</h2><p>{journey.memoryText}</p><span className={`status-pill status-pill--${journey.status}`}>{journey.status === 'playable' ? 'Playable' : 'Preparing beautifully'}</span></div>{index < journeys.length - 1 && <i className="timeline-line"/>}</article>)}</div>{portal && <div className="memory-portal" role="dialog" aria-modal="true"><img src={portal.photos[0]?.generatedAsset ?? portal.coverAsset} alt={portal.photos[0]?.alt}/><div className="memory-portal__glass"><p className="eyebrow">Memory portal</p><h2>{portal.location}</h2><p>{formatJourneyDate(portal.date)} · Age {journeyAge(child, portal)}</p><Button onClick={() => setPortal(null)} variant="glass">Close memory</Button></div></div>}</main>
}
